...
uint8 data_Lengh;       /* length of input data from keypad */
uint8 equal_sign_flag;  /* after you press '=', click any button and the screen will be cleared */

int main()
{
    application_intialize();
     ret = lcd_8bit_send_string_pos(&lcd , 1, 5,"Calculator");
     __delay_ms(500);
    ...
    while(1)
    {
        ...
        {   
            if(!isInputValid(read_data, data_Lengh))
            {
                ...
                calculate_display_result(read_data, data_Lengh);
                equal_sign_flag = 1;
            }
        }
        ...
        else if('c' != keypad_value && (20 != lcd_cursor))
        {
            ...
            read_data[data_Lengh++] = keypad_value;
        }
        else { /* Nothing */ }
    }
